...
public class ManagerService {
    ...
    public void addRestriction(String username, Restriction restriction, String employee_username) {
        ...
        if (emp != null) {
            emp.writelock();
            if ((emp.get_manager() == null) || !emp.get_manager().equals(manager.get_username())) {
                emp.writeunlock();
    ...
    public Restriction getRestriction(String username, String employee_username) {
        ...
        if (emp != null) {
            ...
            if ((emp.get_manager() == null) || !emp.get_manager().equals(username)) {
                ...
                output = new Restriction(emp.get_restriction());
                emp.readunlock();
                return output;
            }
        } else {
    ...
    private void _setPoints(Employee emp, Integer points) {
        if (emp.is_manager()) {
            double ratio = points / emp.get_managerPoints();
    ...
    public int getTotalPoints(String username) {
        ...
        if (emp != null) {
            emp.readlock();
            int points = emp.get_managerPoints();
            emp.readunlock();
            return points;
        } else {
            throw new InvalidParameterException("Employee username doesn't exists");
        ...
        if (emp != null) {
            ...
            for (String emp_username : emp.get_employees()) {
                ...
                if (direct != null) {
                    direct.readlock();
                    EmployeeDetails details = new EmployeeDetails(direct);
                    details.set_restrictions(new Restriction(direct.get_restriction()));
                    output.add(details);
                    direct.readunlock();
                }
            }
